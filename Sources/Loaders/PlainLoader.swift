...
public func load<T: PLoader>(pLoader loader: T, intent: LoaderIntent) -> SectionObservable where T.Context == Void {
  return load(pLoader: loader, intent: intent, context: ())
}

public func load<T: PLoader>(pLoader loader: T, intent: LoaderIntent, context: T.Context) -> SectionObservable {
  do {
