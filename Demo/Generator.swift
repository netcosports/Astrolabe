...
class StyledGenerator<Container: ContainerView,
  ...
  func cellsViews(page: Int, cells: Int) -> [StyledCell<Container, CellView>] {
    let indexes = [Int](1 ... cells)
    let cells: [StyledCell<Container, CellView>] = indexes.map {
      let cell = StyledCell<Container, CellView>(data: TestStyledViewModel(style: .red, title: "\(page) Cell \($0)"))
      cell.id = "\(page) Cell \($0)"
      return cell
    }
    return cells
  ...
  func expandable(page: Int, cells: Int) -> ExpandableCell<Container, CellView> {
    let cells: [StyledCell<Container, CellView>] = cellsViews(page: page, cells: cells)
    let expandable = ExpandableCell<Container, CellView>(data: TestStyledViewModel(style: .red, title: "root \(page)"),
                                                         expandableCells: cells as [Cellable])
    expandable.id = "root \(page)"
    return expandable
  }
...
typealias CollectionStyledGenerator<T1:ReusableView & StyledReusable, T2:ReusableView & Reusable>
  = StyledGenerator<UICollectionView, T1, T2>
where T1.Container == UICollectionView, T1.Data == TestStyledViewModel,
T2.Container == UICollectionView, T2.Data == TestViewModel

typealias TableStyledGenerator<T1:ReusableView & StyledReusable, T2:ReusableView & Reusable>
  = StyledGenerator<UITableView, T1, T2>
where T1.Container == UITableView, T1.Data == TestStyledViewModel,
T2.Container == UITableView, T2.Data == TestViewModel
